...
# And use rake to build changes to haml and coffee files
guard :shell do
  watch(%r{static/.+\.coffee$}) { `rake build:coffee` }
  watch(%r{static/.+\.haml$}) { `rake build:haml` }
  watch(%r{static/libs.+}) { `rake build:libs` }
end

notification :emacs, success: "PaleGreen"
